# Chrome Web Store Listing Copy

This file keeps the public listing text versioned with the repository. Chrome Web Store form fields remain the source submitted to Google, but this is the copy source for future updates.

## v1.1.0

### Summary

```text
Track read, favorite, rated, and unread xkcd comics directly on xkcd.com.
```

### Description

```text
xkcd Reading Tracker is an unofficial browser extension for xkcd. It is not affiliated with, endorsed by, or connected to xkcd or Randall Munroe.

It helps you read through xkcd without losing your place.

The extension adds lightweight tracking controls directly to xkcd.com, plus a compact toolbar popup and a full dashboard for managing progress, favorites, ratings, unread ranges, and settings.

Features:
- Mark comics as read or unread.
- Favorite comics independently from read state.
- Rate comics with stars or a 1-10 control.
- Keep a dedicated continue-reading point.
- Browse all comics, unread comics, or favorites.
- Add optional read/favorite controls into xkcd's own navigation bar.
- Show xkcd alt text below the comic, instantly or after a delay.
- Open Explain xkcd from the comic page.
- Track new xkcd releases with a toolbar badge.
- Use a muted toolbar icon outside xkcd and a full-color icon on valid comic pages.
- Search, sort, filter, preview, rate, and open favorite comics from the dashboard.
- View unread ranges and jump directly to range starts or ends.
- Bulk mark comic numbers or ranges read/unread.
- Run a first-time setup flow to mark older comics read and choose where to continue.
- Export/import JSON backups.
- Use light, dark, or system appearance for the dashboard.

The extension stores reading state in Chrome extension storage. It has no analytics, no account system, no backend server, and no remote code.
```

### Screenshot Set

Chrome Web Store accepts up to five screenshots. Preferred order:

1. `assets/store/screenshots/01-comic-page.png`
2. `assets/store/screenshots/02-popup.png`
3. `assets/store/screenshots/03-dashboard-overview.png`
4. `assets/store/screenshots/04-dashboard-settings.png`
5. `assets/store/screenshots/06-dark-mode-support.png`

Keep `assets/store/screenshots/05-dashboard-diagnostics.png` for GitHub and internal documentation. It is useful, but less important for store conversion than dark-mode support.
